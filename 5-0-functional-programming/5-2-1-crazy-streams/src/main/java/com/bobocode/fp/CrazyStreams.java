...
public class CrazyStreams {
    ...
    public Optional<Account> findRichestPerson() {
        return Optional.of(accounts.stream().max(Comparator.comparing(Account::getBalance)).get());
    ...
    public Map<Boolean, List<Account>> partitionMaleAccounts() {
        return accounts.stream().collect(Collectors.partitioningBy(
               acc -> acc.getSex().equals(Sex.MALE)
        ));
    }
    ...
    public Map<String, List<Account>> groupAccountsByEmailDomain() {
        return accounts.stream().collect(Collectors.groupingBy(
                acc -> acc.getEmail().replaceFirst(".*@(.*)", "$1")
        ));
    }
    ...
    public int getNumOfLettersInFirstAndLastNames() {
        return accounts.stream().mapToInt(
                        acc -> acc.getFirstName().length() + acc.getLastName().length()).sum();
    }
    ...
    public List<Account> sortByFirstAndLastNames() {
        return accounts.stream().sorted(Comparator.comparing(Account::getFirstName)
                .thenComparing(Account::getLastName)).toList();
    }
    ...
    public boolean containsAccountWithEmailDomain(String emailDomain) {
        return accounts.stream().anyMatch(acc -> acc.getEmail().matches(String.format(".*@%s$", emailDomain)));
    }
    ...
    public Map<String, BigDecimal> collectBalancesByEmailForAccountsCreatedOn(int year) {
        return accounts.stream().filter(acc -> acc.getCreationDate().getYear() == year)
    ...
    public Map<Month, String> groupCommaSeparatedFirstNamesByBirthdayMonth() {
        return accounts.stream().collect(
                Collectors.groupingBy(acc -> acc.getBirthday().getMonth(),
